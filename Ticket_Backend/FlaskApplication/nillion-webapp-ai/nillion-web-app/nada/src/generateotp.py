...
def nada_main():
    ...
    timestamp = timestamp / Integer(900)
    combined = timestamp + ticket_code

    hash_value=Integer(5381)
    list_out:list[Output]=[]
    for i in range(1):
        list_out.append(Output(hash_value,"Initialvalue_"+str(i),user))
        hash_value_multiplier = hash_value * Integer(33)
        hash_value_temp = hash_value_multiplier + combined
        power = power1 ** power2
        hash_value = hash_value_temp % power
        list_out.append(Output(hash_value,"Iteration_"+str(i),user))
    final_hash=hash_value*Integer(999)
    system_otp = final_hash % Integer(1000000)
    isvalid=(user_otp==system_otp).if_else(Integer(1),Integer(0))
    out = Output(isvalid, "isvalid", user)
    out2=Output(system_otp, "otp", user)
    out3=Output(final_hash, "HASH", user)
    out4=Output(combined, "combined", user)
    list_out.extend([out,out2,out3,out4])
    return list_out